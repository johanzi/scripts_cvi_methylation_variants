...
## SNP calling with the SHORE pipeline
...
The documentation here is derived from https://github.com/HancockLab/SNP_calling_Arabidopsis?tab=readme-ov-file#shore-pipeline.
...
The VCF file `superVcf_19-07-04_cvis.vcf.b.gz_snps.vcf.b.gz` (matching the `merged.vcf.gz` described before) was generated with the Shore pipeline ([Ossowski et al., 2008](http://genome.cshlp.org/content/18/12/2024). 

The sequencing data of the CVI accessions was published in [Fulgione et al 2022](https://www.nature.com/articles/s41467-022-28800-z) and are available in ENA (accession code PRJEB39079).
...
bcftools view -S df_accessions_83.txt -r Chr1,Chr2,Chr3,Chr4,Chr5 $VCF > subset_83_only_chr.vcf
...
bgzip subset_83_only_chr_biallelic_only_alt_DP3_GQ25_wo_singletons.recode.vcf && tabix subset_83_only_chr_biallelic_only_alt_DP3_GQ25_wo_singletons.recode.vcf.gz
...
```{bash}
# Clone repository with run_gwas_gemma.sh script
...
Create the python script `find_accession.py` to easily find back an accession name based on its sequencing ID (seqID).
...
scale_fill_manual(values=c("#ffffffff", "#949494ff")) +  theme(axis.text.x = element_text(color="black"), 
...
ggplot(data = df_mean_subset, aes(x = pool, y = percent_methylation, group = pool)) + 
  ...
  scale_x_discrete(labels = c("Col-0", "SALK_082977", "FBX5-OE", "Col-3", 
...
# Keep only CHH
df_mean_formated_CHH <- df_mean %>% filter(context == "CHH") %>% filter(location %in% c("Col-3","cmt2-5","SA")) %>% filter(!sample %in% c("4220_AE_cmt2-5","3427_C_Col-0","3599_A_Col-0","4220_Z_Col-0","4220_AG_Col-3"))
...
ggplot(data=df_mean_formated_CHH, aes(x=pool, y=percent_methylation))+ 
  ...
  ylab("% of methylated cytosines (CHH)") + ggtitle("CHH methylation in long TEs") +
  xlab("") + theme_bw() + scale_y_continuous(limits=c(0, 14), breaks=seq(0,14,2)) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
groups <-  df_accessions %>% separate(genotype, c("genotype","rep"),"_rep") %>% mutate_at("genotype", as.factor) %>% pull("genotype")
...
# Plot (use get() to pass the string name of the dataframe as a R object)
# For CG
df_fbx5_CG <- df_mean_TEs_4kb %>% filter(genotype %in% c("fbx5_8_9", "fbx5_3_23","fbx5_12_3","S7_B5")) %>% filter(context == "CpG") %>% droplevels()
...
# Plot (use get() to pass the string name of the dataframe as a R object)
# For CG
df_fbx5_CHG <- df_mean_TEs_4kb %>% filter(genotype %in% c("fbx5_8_9", "fbx5_3_23","fbx5_12_3","S7_B5")) %>% filter(context == "CHG") %>% droplevels()
...
# Plot (use get() to pass the string name of the dataframe as a R object)
df_fbx5_CHH <- df_mean_TEs_4kb %>% filter(genotype %in% c("fbx5_8_9", "fbx5_3_23","fbx5_12_3","S7_B5")) %>% filter(context == "CHH") %>% droplevels()
...
```{r}
# Remove Col-0
coldata_filtered <- coldata  %>% distinct(sample, .keep_all = TRUE) %>% filter(sample!="Col_0") %>% droplevels
...
for(i in 1:100){permutation_DEG_VIM2[i] <- permutate_deseq2("VIM2", cts, coldata)}
...
plotCounts(dds, gene="AT5G39550", intgroup="VIM2", 
...
plotCounts(dds, gene="AT5G39550", intgroup="VIM3", 
