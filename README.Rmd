...
Create methylBase objects based on given list_methylRawLists object. Create DB files if not existing.
...
The paired-end sequencing data of the 190 CVI accessions from Santo Antao were published in Fulgione et al., 2022 (DOI: 10.1038/s41467-022-28800-z) and are available in ENA (accession code PRJEB39079 https://www.ebi.ac.uk/ena/browser/view/PRJEB39079).
...
-S: skip technical reads
-e 6: use 6 threads
--split-files: splits into _1.fastq and _2.fastq
...
The SNP calling is described in the GitHub repository https://github.com/johanzi/SNP_calling_Arabidopsis

## SNP annotation in CVI

Use SNPeff to annotate variants present within Santo Antao island. We have WGS data for 190 accessions.

## Prepare VCF file for the 83 CVI accession

The VCF file `superVcf_19-07-04_cvis.vcf.b.gz_snps.vcf.b.gz` (matching the `merged.vcf.gz` described before) was generated with the Shore pipeline ([Ossowski et al., 2008](http://genome.cshlp.org/content/18/12/2024)). The file is 11 Gb and therefore available only upon request.
...
The output file is be `subset_83_only_chr_biallelic_only_alt_DP3_GQ25_wo_singletons.recode.vcf.gz`.
...
In bash, the rda files generated by the function `load_df_wml` is used to generate an equivalent txt files.
Here is the pipeline in bash

The script `generate_phenotype_gemma.sh` takes as arguments an rda object (generated by `load_df_wml`), the name of the methylation context, the region (gene, TE, ...) and a file containing the accession order as present in the vcf file.
Check GitHub [documentation](https://github.com/HancockLab/WGBS_African_Arabidopsis) for more information.
...
Load R functions for GWAS (originally from https://raw.githubusercontent.com/johanzi/gwas_gemma/refs/heads/master/GWAS_run.R)
...
Top SNP on Chr5 is near VIM3 (790 kb upstream) and falls within the gene AT5G37810.
...
The peak at Chr5 seems to be close to SUVH4. Most significant SNP is 5:5843297
...
We used SnpEff (https://pcingola.github.io/SnpEff/) to define variants and their effect of the 4 candidate genes in Chr5 (AGO9, SUVH4, DRM1, and MET1).


### Subsample VCF file for the 190 Santo Antao accessions

Note: We include Cvi-0 as Santo Antao accession as it is genetically closer to Santo Antao accessions than Fogo accessions. Cvi-0 was collected in 1983 by Lobin in Cape Verde but the island was not specified. See https://www.arabidopsis.org/ais/1983/lobin-1983-aabme.html
...
Outpuf VCF file is `subset_190_SA_alt_DP3_GQ25_wo_singletons_SA_only.recode.vcf`
...
Took 6 min. Output file is `subset_190_SA_alt_DP3_GQ25_wo_singletons_SA_only.recode.ann.vcf`.

The file `snpEff_genes.txt` summarize the number of variants per transcript and variant category. I can import the text file in Excel for easy visualization and sorting.
...
Import text file `summary_SnpEff_chr5_candidates_final.txt` in Excel (Data > From Text > Tab-separated) => Supplementary Table 11
...
Create the python script `scripts/find_accession/find_accession.py` to easily find back an accession name based on its sequencing ID (seqID).
...
![](images/mCHH_long_TEs_FBX5_CMT2.png)
We have 2% of mCHH in FBX5stop vs 1.2% in FBX5ref.
...
Note that all these stats are probably better explained by a linear model integrating the different genes as factor (see part linear model)
...
I need to add manually the status of the deletions for the 83 samples in the VCF file `subset_83_only_chr_biallelic_only_alt_DP3_GQ25_wo_singletons.recode.vcf.gz`
...
kmersGWAS software GitHub repository: https://github.com/voichek/kmersGWAS (version 2.0 used https://github.com/voichek/kmersGWAS/releases/tag/v0.2-beta). Download zip file `v0.2-beta`.
...
The fastq files for all 83 accessions used for GWAS are needed (see Supplementary Table 14 for ENA accession numbers).
...
The plotting was performed on a Jupyter notebook. See [Methylome_plotting_202450610.ipynb](scripts/kmersGWAS/Methylome_plotting_202450610.html).


# DMR analysis for VIM2

To call for differential methylated regions, we pooled the WGBS data of the accessions sharing the same alleles. Our sequencing depth was not enough to use accessions as replicates as we end up with few cytosines that have enough coverage in each sample to perform proper statistics.
...
Select only DMRs with more than 25% of difference between the 2 treatments. Use 5% as threshold for the qvalue
...
Not to loose information about methylation direction, I created a function that average across overlapping DMRs and return a tibble in bed format (see merge_DMRs in `scripts/functions_methylkit.R`)
...
The list of the 7200 PC genes in `list_DMR_PC_genes_VIM2.txt` were copied in the DAVID Functional annotation webtool (https://davidbioinformatics.nih.gov/) (Step 1), using `TAIR_ID` as identifier (Step 2: Select Identifier), then "Gene List" (Step 3: List Type). Gene Ontology results were exported and put in Excel format (Supplementary Table 5).
...
Check if promoter regions are also affected in VIM2del and which are the associated genes. Then define the overlap with DEGs called between VIM2del and VIM2ref.
...
AT2G10608 and AT5G03090 are protein-coding genes while AT5G01055 encodes a long non-coding RNA.
...
We reused the WGBS data generated for WT Col-0 and the *vim* mutants from Stroud et al 2013 (10.1016/j.cell.2012.10.054). BioProject PRJNA172021.
...
WT rep 1 matches the BioSample SAMN03765231 and is splitted into 96 different fastq files (SE 51 bp). Download the SraRunTable on NCBI (Send results to Run selector) and retrieve all SRA that match WT rep1.
...
data:  percent_methylation by VIM2
Bartlett's K-squared = 0.85006, df = 1, p-value = 0.3565


	Two Sample t-test

data:  percent_methylation[CMT2 == 0] and percent_methylation[CMT2 == 1]
t = 33.064, df = 81, p-value < 2.2e-16
alternative hypothesis: true difference in means is not equal to 0
95 percent confidence interval:
 8.016816 9.043461
sample estimates:
...
Upload the files in my local PC to make things faster. I just need the files `*_report_only_chr.txt`. Omit CX files.
...
Linear Hypotheses:
                       Estimate Std. Error t value Pr(>|t|)   
fbx5_8_9 - S7_B5 == 0    2.0900     0.4066   5.140  0.00248 **
fbx5_12_3 - S7_B5 == 0   2.2600     0.4066   5.558  0.00152 **
fbx5_3_23 - S7_B5 == 0   1.0833     0.4066   2.664  0.06841 . 
...
Linear Hypotheses:
                       Estimate Std. Error t value Pr(>|t|)
fbx5_8_9 - S7_B5 == 0   -0.7767     1.3100  -0.593    0.882
fbx5_12_3 - S7_B5 == 0   0.7433     1.3100   0.567    0.894
fbx5_3_23 - S7_B5 == 0   0.7800     1.3100   0.595    0.881
...
VIM3alt (1) is defined as the SNP at position Chr5:15047549, which correspond to the most significant SNP found on Chromosome 5 for gbM GWAS, and located 790 kb downstream of VIM3 (AT5G39550, Chr5:15837178..15840678).
...
Remove Col-0 4559_C and keep first replicate for each accessions with replicates. There is 97 samples left.
...
[AT5G36100](https://www.arabidopsis.org/locus?key=132582) is the most significant and is upregulated in VIM2/4del. SLDP2 (SEED LIPID DROPLET PROTEIN2).Protein of unknown function that is found on the surfaces of lipid droplets and may function to anchor the droplets to the plasma membrane.
...
To quantify the expression of transposable elements (TEs) in our RNA-seq dataset, we used the RepEnrich2 pipeline (https://github.com/nerettilab/RepEnrich2)

## TE annotation

`TAIR10_Transposable_Elements.txt` can be downloaded on https://www.arabidopsis.org/download/file?path=Genes/TAIR10_genome_release/TAIR10_transposable_elements/TAIR10_Transposable_Elements.txt
...
Column 5: Class (family)
Column 6: Family (superfamily)
...
Get reference genome from https://www.arabidopsis.org/download/list?dir=Genes%2FTAIR10_genome_release and build bowtie2 indexes
...
For each TE, a fasta file and bowtie2 index files (bt and bt2) are created. The file `repgenomes_key.txt` contains the name of each TEs and a unique ID (digit). `repnames.bed` Contains the position of each TE. This step is performed only once.
...
bowtie2 and bedtools also need to be installed. Used trimmed reads from the RNA-seq (see RNA-seq part).
...
The `RepEnrich2_subset.py` script should be run to output discrete files for uniquely and multi-mapping reads. MAPQ threshold of 30. Retrieve uniquely mapped read using the command `samtools view -h -F 4 -bq 30 file.bam`. The q of -bq selects for uniquely mapped reads (quality >=30) as mapping quality of multimapping reads = 0 if I am correct. 

Also, the script selects multimapping reads by first retrieving all mapped reads (`"samtools view -h -F 4 -b file.bam`). Then `"samtools view -U _multimap_filtered.bam -bq 30 _map.bam`, which retrieves all reads not matching -q 30 thanks to parameter -U `-U FILE  output reads not selected by filters to FILE`

It also converts the _multimap_filtered.bam file into a fastq file using command `bedtools bamtofastq -i _multimap_filtered.bam -fq _multimap.fastq`.
...
Most replicates cluster together for the different accessions but it less clustered than for gene expression.
Remove replicates to keep only one sample per accession. Remove Col-0 from the analysis. We end up with 97 samples.
...
Check how many TEs are differentially expressed when permutating alleles within populations.
...
Observed values (horizontal red lines) are above the permutation distribution.


# Analysis of presence/absence variation of TEs

Going through the literature, several methods are used to look for TE presence/absence variation (PAV) based on paired-end sequencing. The two most recent methods I found are TE-Tracker [Gilly et al 2014](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/s12859-014-0377-z) and TEPID [Stuart et al 2016](https://elifesciences.org/articles/20777). It seems the documentation is more complete for TEPID and the method was used in a recent paper on *Capsella rubella* ([Niu et al 2019](https://doi.org/10.1073/pnas.1811498116)). 

## Installation TEPID

I fixed some bugs in the TEPID but not all my changes we integrated in the original github repository (https://github.com/ListerLab/TEPID). I recommend therefore to download the version I forked on my GitHub account (https://github.com/johanzi/TEPID).
...
Create Yaha index. They don't give parameters for indexing https://elifesciences.org/articles/20777 so I use the default ones: -H (max-Hits) 65525, -L (seed-Length) 15, -S (Skip-distance) 1
...
The TAIR9 TE annotation (provided by TEPID in `TEPID/Annotation/Arabidopsis/TAIR9_TE.bed.gz`) seems similar to Araport11 (same number of features, positions of ATCOPIA24 similar).

Make sure the chromosome name in the TE bed file and the genome fasta files are the same (for instance chr or Chr). If needed changed bed file to fit the fasta file chromosome name system
...
Check insert size of the paired-end reads (use Python script getinsertsize.py available in gist https://gist.github.com/davidliwei/2323462)
...
Create a wrap up bash script `run_tepid.sh` that takes as input unmapped bam files containing both read1 and read2:
...
Launch the pipeline for all CVI. Here I use the job manager LSF. Adapt script to your HPC manager if different (e.g. SLURM). I allocated 20 CPUs and 12Gb of RAM per job.
...
Copy paste seqID for FBX5 from the Supp. Table Excel file.

```{bash, eval=FALSE}

while read i; do
  cat deletions_${i}.bed >> summary/deletions_FBX5_0.bed
done < summary/list_FBX5_0.txt

while read i; do
  cat insertions_${i}.bed >> summary/insertions_FBX5_0.bed
done < summary/list_FBX5_0.txt

while read i; do
  cat deletions_${i}.bed >> summary/deletions_FBX5_1.bed
done < summary/list_FBX5_1.txt

while read i; do
  cat insertions_${i}.bed >> summary/insertions_FBX5_1.bed
done < summary/list_FBX5_1.txt

# Now sort and merge
bedtools sort -i deletions_FBX5_0.bed | cut -f1-5 | uniq > deletions_FBX5_0.sorted.bed
bedtools sort -i deletions_FBX5_1.bed | cut -f1-5 | uniq > deletions_FBX5_1.sorted.bed

# More complicated for insertions as the coordinates are not exactly the same
bedtools sort -i insertions_FBX5_1.bed | cut -f1,2,3,7  | uniq > insertions_FBX5_1.sorted.bed
bedtools sort -i insertions_FBX5_0.bed | cut -f1,2,3,7 | uniq > insertions_FBX5_0.sorted.bed

wc -l deletions_FBX5_*.sorted.bed
2039 deletions_FBX5_0.sorted.bed
2017 deletions_FBX5_1.sorted.bed


# Not in common
bedtools intersect -v -a deletions_FBX5_0.bed -b deletions_FBX5_1.bed > deletions_unique_FBX5_0.bed
bedtools intersect -v -a deletions_FBX5_1.bed -b deletions_FBX5_0.bed > deletions_unique_FBX5_1.bed

wc -l deletions_unique_FBX5_*
  303 deletions_unique_FBX5_0.bed
  272 deletions_unique_FBX5_1.bed

```

303 deletions unique to FBX5ref, 284 deletions unique to FBX5stop.

127 accessions FBX5ref and 62 accessions with FBX5stop.


```{bash, eval=FALSE}

while read i; do
  cat deletions_${i}.bed >> summary/deletions_CMT2_0.bed
done < summary/list_CMT2_0.txt

while read i; do
  cat insertions_${i}.bed >> summary/insertions_CMT2_0.bed
done < summary/list_CMT2_0.txt

while read i; do
  cat deletions_${i}.bed >> summary/deletions_CMT2_1.bed
done < summary/list_CMT2_1.txt

while read i; do
  cat insertions_${i}.bed >> summary/insertions_CMT2_1.bed
done < summary/list_CMT2_1.txt

# Now sort and merge
bedtools sort -i deletions_CMT2_0.bed | cut -f1-5 | uniq > deletions_CMT2_0.sorted.bed
bedtools sort -i deletions_CMT2_1.bed | cut -f1-5 | uniq > deletions_CMT2_1.sorted.bed


bedtools sort -i insertions_CMT2_1.bed | cut -f1,2,3,7  | uniq > insertions_CMT2_1.sorted.bed
bedtools sort -i insertions_CMT2_0.bed | cut -f1,2,3,7 | uniq > insertions_CMT2_0.sorted.bed


wc -l summary/deletions_CMT2_0.sorted.bed
2110 summary/deletions_CMT2_0.sorted.bed

wc -l summary/deletions_CMT2_1.sorted.bed
1820 summary/deletions_CMT2_1.sorted.bed


# Not in common 
bedtools intersect -v -a deletions_CMT2_0.sorted.bed -b deletions_CMT2_1.sorted.bed > deletions_unique_CMT2_0.bed
bedtools intersect -v -a deletions_CMT2_1.sorted.bed -b deletions_CMT2_0.sorted.bed > deletions_unique_CMT2_1.bed

wc -l deletions_unique_CMT2_*
  383 deletions_unique_CMT2_0.bed
   99 deletions_unique_CMT2_1.bed

```

383 deletions unique to CMT2ref and 99 deletions unique to CMT2stop. It makes sense as there are only 60 accessions with the CMT2stop allele while 130 accessions have the CMT2ref allele.

Check by permutation


```{r}

# Permutation CMT2stop (60 accessions)

for j in {1..20}; do
  shuf all_190_seqid.txt | head -n 60 > summary/permutations_CMT2/rand_60_seqid_${j}.txt
  while read i; do
    cat deletions_${i}.bed 
  done < summary/permutations_CMT2/rand_60_seqid_${j}.txt | bedtools sort -i - | cut -f1-5 | uniq > summary/permutations_CMT2/deletions_rand_60_${j}.bed
done

wc -l summary/permutations_CMT2/deletions_rand_60_*
   2011 summary/permutations_CMT2/deletions_rand_60_1.bed
   2017 summary/permutations_CMT2/deletions_rand_60_10.bed
   1939 summary/permutations_CMT2/deletions_rand_60_11.bed
   1925 summary/permutations_CMT2/deletions_rand_60_12.bed
   1966 summary/permutations_CMT2/deletions_rand_60_13.bed
   1945 summary/permutations_CMT2/deletions_rand_60_14.bed
   1919 summary/permutations_CMT2/deletions_rand_60_15.bed
   1966 summary/permutations_CMT2/deletions_rand_60_16.bed
   1887 summary/permutations_CMT2/deletions_rand_60_17.bed
   2025 summary/permutations_CMT2/deletions_rand_60_18.bed
   2005 summary/permutations_CMT2/deletions_rand_60_19.bed
   2010 summary/permutations_CMT2/deletions_rand_60_2.bed
   1896 summary/permutations_CMT2/deletions_rand_60_20.bed
   1930 summary/permutations_CMT2/deletions_rand_60_3.bed
   1890 summary/permutations_CMT2/deletions_rand_60_4.bed
   1981 summary/permutations_CMT2/deletions_rand_60_5.bed
   1958 summary/permutations_CMT2/deletions_rand_60_6.bed
   2021 summary/permutations_CMT2/deletions_rand_60_7.bed
   2038 summary/permutations_CMT2/deletions_rand_60_8.bed
   1873 summary/permutations_CMT2/deletions_rand_60_9.bed


# Permutation CMT2ref (130 accessions)
   
for j in {1..20}; do
  shuf all_190_seqid.txt | head -n 130 > summary/permutations_CMT2/rand_130_seqid_${j}.txt
  while read i; do
    cat deletions_${i}.bed 
  done < summary/permutations_CMT2/rand_130_seqid_${j}.txt | bedtools sort -i - | cut -f1-5 | uniq > summary/permutations_CMT2/deletions_rand_130_${j}.bed
done

wc -l summary/permutations_CMT2/deletions_rand_130_*
   2113 summary/permutations_CMT2/deletions_rand_130_1.bed
   2142 summary/permutations_CMT2/deletions_rand_130_10.bed
   2141 summary/permutations_CMT2/deletions_rand_130_11.bed
   2146 summary/permutations_CMT2/deletions_rand_130_12.bed
   2142 summary/permutations_CMT2/deletions_rand_130_13.bed
   2142 summary/permutations_CMT2/deletions_rand_130_14.bed
   2150 summary/permutations_CMT2/deletions_rand_130_15.bed
   2170 summary/permutations_CMT2/deletions_rand_130_16.bed
   2127 summary/permutations_CMT2/deletions_rand_130_17.bed
   2080 summary/permutations_CMT2/deletions_rand_130_18.bed
   2141 summary/permutations_CMT2/deletions_rand_130_19.bed
   2138 summary/permutations_CMT2/deletions_rand_130_2.bed
   2144 summary/permutations_CMT2/deletions_rand_130_20.bed
   2118 summary/permutations_CMT2/deletions_rand_130_3.bed
   2147 summary/permutations_CMT2/deletions_rand_130_4.bed
   2126 summary/permutations_CMT2/deletions_rand_130_5.bed
   2108 summary/permutations_CMT2/deletions_rand_130_6.bed
   2111 summary/permutations_CMT2/deletions_rand_130_7.bed
   2160 summary/permutations_CMT2/deletions_rand_130_8.bed
   2103 summary/permutations_CMT2/deletions_rand_130_9.bed

   
# Deletions unique to permuted CMT2stop
for i in {1..20}; do   
   bedtools intersect -v -a summary/permutations_CMT2/deletions_rand_60_${i}.bed -b summary/permutations_CMT2/deletions_rand_130_${i}.bed | wc -l
done | sort -n
8
9
14
...
# Deletions unique to permuted CMT2ref
for i in {1..20}; do
  bedtools intersect -v -a summary/permutations_CMT2/deletions_rand_130_${i}.bed -b summary/permutations_CMT2/deletions_rand_60_${i}.bed | wc -l
done | sort -n
...
while read i; do
mv *_${i}.* SA_data
done < list_190_seqid.txt

cd SA_data

cat deletions_*.bed | cut -f1-5 | sort | uniq > all_deletions.bed

wc -l all_deletions.bed
2209 all_deletions.bed

# Now check for each accessions if each deletions is present or absent
cp ../list_190_seqid.txt .

while read j; do
  while read i; do
    if [[ `grep -w "$i" deletions_${j}.bed` ]]; then
      echo "1" >> summary_deletions_${j}.txt
    else
      echo "0" >> summary_deletions_${j}.txt
    fi
  done < all_deletions.bed
done < list_190_seqid.txt

paste summary_deletions_*txt > matrix_deletions_190.txt

awk '{print NF; exit}' matrix_deletions_190.txt
190


# add unique deletion number as ID
for i in {1..2209}; do
  del=$(sed -n ${i}p all_deletions.bed)
  echo -e "${del}\tID_${i}"
done > all_deletions_with_ID.bed

cut -f1 all_deletions_with_ID.bed | sort | uniq -c
    628 Chr1
    332 Chr2
    419 Chr3
    390 Chr4
...
cut -f6 all_deletions_with_ID.bed | paste - matrix_deletions_190.txt > matrix_deletions_190_with_ID.txt
...
```{bash, eval=FALSE}
# For insertions, it is more complicated because I need to look at overlap (>80%)

# Check https://plink.readthedocs.io/en/latest/bedtools_int/
cat insertions_*.bed | cut -f1,2,3,7 | sort | uniq > all_insertions.bed

# Collapse overlapping feature and merge names if not different
bedtools sort -i all_insertions.bed | bedtools merge -i - -c 4 -o distinct > all_insertions.merged.bed

wc -l all_insertions.merged.bed
4899 all_insertions.merged.bed


# Get coordinates of the insertion and check if present in each accession

for i in {1..4899}; do
  sed -n ${i}p all_insertions.merged.bed > tmp.bed
  while read j; do
    if [[ `bedtools intersect -a tmp.bed -b deletions_${j}.bed -f 0.8` ]]; then
      echo "1" >> summary_insertions_${j}.txt
    else
      echo "0" >> summary_insertions_${j}.txt
    fi
  done < list_190_seqid.txt
  rm tmp.bed
done

paste summary_insertions_*txt > matrix_insertions_190.txt

awk '{print NF; exit}' matrix_insertions_190.txt
190

# Add ID 
for i in {1..4899}; do
  ins=$(sed -n ${i}p all_insertions.merged.bed)
  echo -e "${ins}\tID_${i}"
done > all_insertions_with_ID.merged.bed

cut -f1 all_insertions_with_ID.merged.bed | sort | uniq -c
   1050 Chr1
   1012 Chr2
    993 Chr3
    821 Chr4
  ...
  cut -f5 all_insertions_with_ID.merged.bed | paste - matrix_insertions_190.txt > matrix_insertions_190_with_ID.txt
...
}' list_190_seqid.txt >> header_matrix.txt

# Change space into tab
cat header_matrix.txt | tr -s " " '\t' > header_matrix1.txt
mv header_matrix1.txt header_matrix.txt

# Add "ID" manually as first col

# Create matrix files with headers
cat header_matrix.txt matrix_insertions_190_with_ID.txt > matrix_insertions_190_with_ID_with_header.txt
cat header_matrix.txt matrix_deletions_190_with_ID.txt > matrix_deletions_190_with_ID_with_header.txt

# Copy files into project folder
cp matrix_deletions_190_with_ID_with_header.txt matrix_insertions_190_with_ID_with_header.txt all_insertions_with_ID.merged.bed all_deletions_with_ID.bed ~/SCRIPTS/TEs_analysis/TEPID_output

```



### Analysis segregating TEs

```{r}

matrix_deletions_SA <- read.delim("data/tepid/matrix_deletions_190_with_ID_with_header.txt", row.names=1)

matrix_insertions_SA <- read.delim("data/tepid/matrix_insertions_190_with_ID_with_header.txt", row.names=1)

ID_deletions <- read.delim("data/tepid/all_deletions_with_ID.bed", header = F)
colnames(ID_deletions) <- c("chr","start","end","strand","TE","ID")
ID_deletions$ID <- as.factor(ID_deletions$ID)

ID_insertions <- read.delim("data/tepid/all_insertions_with_ID.merged.bed", header = F)
colnames(ID_insertions) <- c("chr","start","end","TE","ID")
ID_insertions$ID <- as.factor(ID_insertions$ID)

```


Filter TEs that segregate:

```{r}
# 2177/2209 TE deletions segregate
sum(rowSums(matrix_deletions_SA) != 190)

# 4897/4899 are segregating
sum(rowSums(matrix_insertions_SA) != 190)

```

### Type of TEs

Check the type of TEs.

```{r}
df_TEs <- read.delim("data/tepid/TAIR10_Transposable_Elements_RepEnrich.txt", header=FALSE)

colnames(df_TEs) <- c("chr","start","end","TE","family","superfamily")

deletions_SA_TE <- merge.data.frame(ID_deletions, df_TEs, by="TE")

insertions_SA_TE <- merge.data.frame(ID_insertions, df_TEs, by="TE")

```

Get summary of TE id with family
...
Status allele for the 190 accessions for the 3 variants.For 12849, FBX5 call was heterozygous and was then assigned "NA". To avoid bug, I assigned 0 in the allele_id.txt file.

```{r}

allele_id <- read.delim("data/tepid/allele_id.txt")
allele_id$seqIDX <- paste("X",allele_id$seqID,sep="")
allele_id$seqIDX <- as.factor(allele_id$seqIDX)

# Check if ID fit in df and matrix
#allele_id$seqIDX == colnames(matrix_deletions_SA)
#allele_id$seqIDX == colnames(matrix_insertions_SA)
...
idx <- allele_id$FBX5=="0" # Does not work with FBX5 due to the NA 

hist(rowSums(matrix_deletions_SA[,allele_id$FBX5=="0"]))
hist(rowSums(matrix_deletions_SA[,allele_id$FBX5=="1"]))

hist(rowSums(matrix_insertions_SA[,allele_id$FBX5=="0"]))
hist(rowSums(matrix_insertions_SA[,allele_id$FBX5=="1"]))

```

```{r}
hist(rowSums(matrix_deletions_SA[,allele_id$CMT2=="0"]))
hist(rowSums(matrix_deletions_SA[,allele_id$CMT2=="1"]))


hist(rowSums(matrix_insertions_SA[,allele_id$CMT2=="0"]))
hist(rowSums(matrix_insertions_SA[,allele_id$CMT2=="1"]))

hist(rowSums(matrix_deletions_SA))
```

```{r}
df_TEs_superfamily <- df_TEs %>%
  group_by(superfamily) %>%
  summarise(counts = n()) 

# Stacked bar plots of y = counts by x = cut,
# colored by the variable color
ggplot(df_TEs_superfamily, aes(x = superfamily, y = counts)) +
  geom_bar(stat = "identity", position = position_stack()) + theme_bw() + theme(axis.text.x = element_text(angle=90, hjust=1)) + ggtitle("31,189 TEs TAIR10")

```


```{r}
deletions_SA_TE_summary <- deletions_SA_TE %>%
  group_by(superfamily) %>%
  summarise(counts = n()) 

ggplot(deletions_SA_TE_summary, aes(x = superfamily, y = counts)) +
  geom_bar(stat = "identity", position = position_stack()) + theme_bw() +theme(axis.text.x = element_text(angle=90, hjust=1)) + ggtitle("2209 deletions in SA")

```

```{r}
insertions_SA_TE_summary <- insertions_SA_TE %>%
  group_by(superfamily) %>%
  summarise(counts = n())

ggplot(insertions_SA_TE_summary, aes(x = superfamily, y = counts)) +
  geom_bar(stat = "identity", position = position_stack()) + theme_bw() +theme(axis.text.x = element_text(angle=90, hjust=1)) + ggtitle("4899 insertions in SA")
```
...
```{r}
insertions_SA_TE_summary$category <- "insertions"
deletions_SA_TE_summary$category <- "deletions"

SA_TE_summary <- rbind(insertions_SA_TE_summary, deletions_SA_TE_summary)

ggplot(SA_TE_summary, aes(x = superfamily, y = counts, fill=category)) +
  geom_bar(position="dodge",stat = "identity") + theme_bw() +theme(axis.text.x = element_text(angle=90, hjust=1)) + ggtitle("TE insertions and deletions in SA") +  theme(axis.text.x = element_text(color="black"), 
      axis.text.y = element_text(color="black"),
      axis.ticks = element_line(color = "black")) + 
      theme(plot.title = element_text(hjust = 0.5)) + 
      scale_y_continuous(labels = scales::comma_format())
...
It seems the LTR_Gypsy family is enriched for insertions

```{r}

insertions_SA_TE_summary <- insertions_SA_TE %>% filter(superfamily=="LTR_Gypsy") %>%
  group_by(family) %>%
  summarise(counts = n())

deletions_SA_TE_summary <- deletions_SA_TE %>% filter(superfamily=="LTR_Gypsy") %>%
  group_by(family) %>%
  summarise(counts = n()) 
...
ggplot(SA_TE_summary, aes(x = family, y = counts, fill = category)) +
  geom_bar(position = "dodge", stat = "identity") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  ggtitle("TE insertions and deletions in SA") +
  theme(
...
```{r}
insertions_SA_TE %>% filter(family=="ATHILA6A")
```
...
insertions_SA_TE_summary <- insertions_SA_TE %>% filter(family=="ATHILA6A") %>%
  group_by(family) %>%
  summarise(counts = n())

deletions_SA_TE_summary <- deletions_SA_TE %>% filter(family=="ATHILA6A") %>%
  group_by(family) %>%
  summarise(counts = n()) 
...
Real values of insertions and deletions:

```{r}
CMT2_0_del <- rowSums(matrix_deletions_SA[,allele_id$CMT2=="0"])
CMT2_1_del <- rowSums(matrix_deletions_SA[,allele_id$CMT2=="1"])

CMT2_0_ins <- rowSums(matrix_insertions_SA[,allele_id$CMT2=="0"])
CMT2_1_ins <- rowSums(matrix_insertions_SA[,allele_id$CMT2=="1"])


CMT2_0_del <- colSums(matrix_deletions_SA[,allele_id$CMT2=="0"])
CMT2_1_del <- colSums(matrix_deletions_SA[,allele_id$CMT2=="1"])

CMT2_0_ins <- colSums(matrix_insertions_SA[,allele_id$CMT2=="0"])
CMT2_1_ins <- colSums(matrix_insertions_SA[,allele_id$CMT2=="1"])
```

Permutation:

```{r}
names_id <- colnames(matrix_deletions_SA)[allele_id$CMT2=="0"]
matrix_deletions_SA_CMT2_0 <- matrix_deletions_SA[names_id]

list1 <- c()
for (i in 1:1000){
  list1[i] <- (mean(colSums(matrix_deletions_SA_CMT2_0[,sample(names_id, 60)])))
}

plot_permutation(list1, mean(CMT2_1_del), ylab = "Mean # TE deletions/accession") + ggtitle("Permutation deletions CMT2") + theme(plot.title = element_text(hjust = 0.5))
...
```{r}
names_id <- colnames(matrix_insertions_SA)[allele_id$CMT2=="0"]
matrix_insertions_SA_CMT2_0 <- matrix_insertions_SA[names_id]

list1 <- c()
for (i in 1:1000){
  list1[i] <- (mean(colSums(matrix_insertions_SA_CMT2_0[,sample(names_id, 60)])))
}

plot_permutation(list1, mean(CMT2_1_ins), ylab = "Mean # TE insertions/accession") + ggtitle("Permutation insertions CMT2") + theme(plot.title = element_text(hjust = 0.5))
...
```{r}
FBX5_0_del <- colSums(matrix_deletions_SA[,allele_id$FBX5=="0"])
FBX5_1_del <- colSums(matrix_deletions_SA[,allele_id$FBX5=="1"])

FBX5_0_ins <- colSums(matrix_insertions_SA[,allele_id$FBX5=="0"])
FBX5_1_ins <- colSums(matrix_insertions_SA[,allele_id$FBX5=="1"])
...
```{r}
names_id <- colnames(matrix_deletions_SA)[allele_id$FBX5=="0"]
matrix_deletions_SA_FBX5_0 <- matrix_deletions_SA[names_id]

list1 <- c()
for (i in 1:1000){
  list1[i] <- (mean(colSums(matrix_deletions_SA_FBX5_0[,sample(names_id, 62)])))
}

plot_permutation(list1, mean(FBX5_1_del), ylab = "Mean # TE deletions/accession") + ggtitle("Permutation deletions FBX5") + theme(plot.title = element_text(hjust = 0.5))
```

```{r}
names_id <- colnames(matrix_insertions_SA)[allele_id$FBX5=="0"]
matrix_insertions_SA_FBX5_0 <- matrix_insertions_SA[names_id]

list1 <- c()
for (i in 1:1000){
  list1[i] <- (mean(colSums(matrix_insertions_SA_FBX5_0[,sample(names_id, 62)])))
}

plot_permutation(list1, mean(FBX5_1_ins), ylab = "Mean # TE insertions/accession") + ggtitle("Permutation insertions FBX5") + theme(plot.title = element_text(hjust = 0.5))
```

## Analysis all TEs

Import this file in Excel and merge with the genotyping call for VIM2, CMT2, and FBX5 (supplementary table X) also put in text file `data/TE_indel_TEPID.txt`.
...
df_TE_SA$accession <- factor(df_TE_SA$accession , levels=df_TE_SA$accession[order(df_TE_SA$TE_deletions)])

ggplot(df_TE_SA, aes(accession, TE_deletions)) + geom_bar(aes(x=accession, y=TE_deletions, fill=CMT2), position="dodge", stat="identity") + ggtitle("TE deletions") + theme(axis.text.x = element_text(color="black"), 
    axis.text.y = element_text(color="black"),
    axis.ticks = element_line(color = "black")) + 
    theme(plot.title = element_text(hjust = 0.5)) + 
    scale_y_continuous(labels = scales::comma_format()) + 
...
df_TE_SA$accession <- factor(df_TE_SA$accession , levels=df_TE_SA$accession[order(df_TE_SA$TE_insertions)])

ggplot(df_TE_SA, aes(accession, TE_insertions)) + geom_bar(aes(x=accession, y=TE_insertions, fill=CMT2), position="dodge", stat="identity") + ggtitle("TE insertions")+  theme(axis.text.x = element_text(color="black"), 
    axis.text.y = element_text(color="black"),
    axis.ticks = element_line(color = "black")) + 
    theme(plot.title = element_text(hjust = 0.5)) + 
    scale_y_continuous(labels = scales::comma_format()) +
...
ggplot(df_TE_SA, aes(accession, TE_deletions)) + geom_bar(aes(x=accession, y=TE_deletions, fill=FBX5), position="dodge", stat="identity") + ggtitle("TE deletions") + theme(axis.text.x = element_text(color="black"), 
    axis.text.y = element_text(color="black"),
    axis.ticks = element_line(color = "black")) + 
    theme(plot.title = element_text(hjust = 0.5)) + 
    scale_y_continuous(labels = scales::comma_format()) + 
...
ggplot(df_TE_SA, aes(accession, TE_insertions)) + geom_bar(aes(x=accession, y=TE_insertions, fill=FBX5), position="dodge", stat="identity") + ggtitle("TE insertions")+  theme(axis.text.x = element_text(color="black"), 
    axis.text.y = element_text(color="black"),
    axis.ticks = element_line(color = "black")) + 
    theme(plot.title = element_text(hjust = 0.5)) + 
    scale_y_continuous(labels = scales::comma_format()) +
...
ggplot(df_TE_SA, aes(accession, TE_deletions)) + geom_bar(aes(x=accession, y=TE_deletions, fill=VIM2), position="dodge", stat="identity") + ggtitle("TE deletions") + theme(axis.text.x = element_text(color="black"), 
    axis.text.y = element_text(color="black"),
    axis.ticks = element_line(color = "black")) + 
    theme(plot.title = element_text(hjust = 0.5)) + 
    scale_y_continuous(labels = scales::comma_format()) + 
...
ggplot(df_TE_SA, aes(accession, TE_insertions)) + geom_bar(aes(x=accession, y=TE_insertions, fill=VIM2), position="dodge", stat="identity") + ggtitle("TE insertions")+  theme(axis.text.x = element_text(color="black"), 
    axis.text.y = element_text(color="black"),
    axis.ticks = element_line(color = "black")) + 
    theme(plot.title = element_text(hjust = 0.5)) + 
    scale_y_continuous(labels = scales::comma_format()) +
...
```{r}
df_TE_SA %>% filter(FBX5!="NA") %>% ggplot(aes_string(x="CMT2", y="TE_deletions", color="FBX5")) +
    theme_bw()+
    geom_boxplot() + ggtitle("TE deletions") +
...
```
```{r}
df_TE_SA %>% filter(FBX5!="NA") %>% ggplot(aes_string(x="CMT2", y="TE_insertions", color="FBX5")) +
    theme_bw()+
    geom_boxplot() + ggtitle("TE insertions") +
    geom_point(position=position_jitterdodge(), size=1) +
    theme(plot.title = element_text(hjust = 0.5))
...
```{r}
df_TE_SA %>% filter(FBX5!="NA") %>% ggplot(aes_string(x="CMT2", y="TE_insertions", color="VIM2")) +
    theme_bw()+
    geom_boxplot() + ggtitle("TE insertions") +
    geom_point(position=position_jitterdodge(), size=1) +
    theme(plot.title = element_text(hjust = 0.5))

df_TE_SA %>% filter(FBX5!="NA") %>% ggplot(aes_string(x="CMT2", y="TE_deletions", color="VIM2")) +
    theme_bw()+
    geom_boxplot() + ggtitle("TE deletions") +
    geom_point(position=position_jitterdodge(), size=1) +
    theme(plot.title = element_text(hjust = 0.5))
...
```{r}
df_TE_SA %>% filter(FBX5!="NA") %>% ggplot(aes_string(x="FBX5", y="TE_insertions", color="VIM2")) +
    theme_bw()+
    geom_boxplot() + ggtitle("TE insertions") +
    geom_point(position=position_jitterdodge(), size=1) +
    theme(plot.title = element_text(hjust = 0.5))

df_TE_SA %>% filter(FBX5!="NA") %>% ggplot(aes_string(x="FBX5", y="TE_deletions", color="VIM2")) +
    theme_bw()+
    geom_boxplot() + ggtitle("TE deletions") +
    geom_point(position=position_jitterdodge(), size=1) +
    theme(plot.title = element_text(hjust = 0.5))
...
df_TE_SA_long_deletions <- df_TE_SA %>% select(TE_deletions, accession, VIM2, CMT2, FBX5)

df_TE_SA_long_deletions$VIM2 <- as.character(df_TE_SA_long_deletions$VIM2)
df_TE_SA_long_deletions$CMT2 <- as.character(df_TE_SA_long_deletions$CMT2)
df_TE_SA_long_deletions$FBX5 <- as.character(df_TE_SA_long_deletions$FBX5)

df_TE_SA_long_deletions['VIM2'][df_TE_SA_long_deletions['VIM2'] == "1"] <- "VIM2del"
df_TE_SA_long_deletions['VIM2'][df_TE_SA_long_deletions['VIM2'] == "0"] <- "VIM2ref"

df_TE_SA_long_deletions['CMT2'][df_TE_SA_long_deletions['CMT2'] == "1"] <- "CMT2stop"
df_TE_SA_long_deletions['CMT2'][df_TE_SA_long_deletions['CMT2'] == "0"] <- "CMT2ref"

df_TE_SA_long_deletions['FBX5'][df_TE_SA_long_deletions['FBX5'] == "1"] <- "FBX5stop"
df_TE_SA_long_deletions['FBX5'][df_TE_SA_long_deletions['FBX5'] == "0"] <- "FBX5ref"

test <- pivot_longer(df_TE_SA_long_deletions, cols=VIM2:FBX5,
                                        names_to="gene",
                                        values_to="allele")

table(test$TE_deletions, test$allele)
...
Significant difference (p-value = 0.2%).
...
matrix_deletions_TE <- matrix_deletions_SA %>% rownames_to_column("ID")

matrix_deletions_TE$ID <- as.factor(matrix_deletions_TE$ID)

deletions_SA_TE_ID <- deletions_SA_TE %>% dplyr::select(ID, TE, family, superfamily)

matrix_deletions_TE_2 <- merge(matrix_deletions_TE, deletions_SA_TE_ID, by="ID")

matrix_deletions_TE_2 <- matrix_deletions_TE_2 %>% dplyr::select(c(ID, TE, family, superfamily), everything())

matrix_deletions_TE_2$TE <-as.factor(matrix_deletions_TE_2$TE)
matrix_deletions_TE_2$family <-as.factor(matrix_deletions_TE_2$family)
matrix_deletions_TE_2$superfamily <-as.factor(matrix_deletions_TE_2$superfamily)
...
for (i in 5:194){
  matrix_deletions_TE_2[,i]
}

test <- matrix_deletions_TE_2[,1:5]


group_by(test, superfamily) %>%
  summarise(
    count = n(),
    sum = sum(X12766, na.rm = TRUE),
  )

test <- matrix_deletions_TE_2[,c(1,2,3,4,6)]


tb1 <- group_by(test, superfamily) %>%
  summarise(
    count = n(),
    sum = sum(X12849, na.rm = TRUE),
  )

summarize_TE <- function(index, matrix){
  m1 <- matrix[,c(4,index)]
  accession <- names(m1)[2]
  tb1 <- group_by(m1, superfamily) %>%
  summarise(
    sum = sum(get(accession), na.rm = TRUE),
  )
  return(tb1$sum)
}


df1 <- data.frame(matrix(ncol = 190, nrow = 18))
colnames(df1) <- names(matrix_deletions_TE_2)[5:194]

for (i in 5:194){
  j=i-4
  df1[[j]] <- summarize_TE(i, matrix_deletions_TE_2)
}

df1$superfamily <- levels(matrix_deletions_TE_2$superfamily)

# Convert to long
df1_long <- df1 %>% gather(accession, number_TE, 1:190)

df1_long$superfamily <- as.factor(df1_long$superfamily)
df1_long$accession <- as.factor(df1_long$accession)

# Encode CMT2 and FBX5 status
df1_long_code <- merge(df1_long, allele_id, by.x="accession", by.y ="seqIDX")

df1_long_code <- df1_long_code %>% mutate_at(c("VIM2","CMT2","FBX5"), as.factor) 

```

```{r}

ggboxplot(df1_long_code, "superfamily", "number_TE",
  color = "CMT2",
  xlab = "TE superfamily", ylab = "Number of TE deletions", title = "TE deletions across super family",
  palette = col,
...
ggboxplot(df1_long_code, "superfamily", "number_TE", color="FBX5", 
          xlab="TE superfamily", ylab="Number of TE deletions", title="TE deletions across super family",
          palette=col, 
          add.params = list(size = 0.7, alpha = 0.2), outlier.shape = NA) + 
  rotate_x_text(45) + theme(plot.title = element_text(hjust = 0.5))
...
ggboxplot(df1_long_code, "superfamily", "number_TE", color="VIM2", 
          xlab="TE superfamily", ylab="Number of TE deletions", title="TE deletions across super family",
          palette=col, 
          add.params = list(size = 0.7, alpha = 0.2), outlier.shape = NA) + 
  rotate_x_text(45) + theme(plot.title = element_text(hjust = 0.5))
...
# ###############################
## Connection with RNA-seq data

Check if correlation between transposition and expression from the RNA-seq data
...
ID_del_sig_TE_CMT2 <- as.vector(ID_deletions[ID_deletions$TE %in% sig_TE_CMT2$geneID, 6])
# 9 TEs found

ID_del_sig_TE_FBX5 <- as.vector(ID_deletions[ID_deletions$TE %in% sig_TE_FBX5$geneID, 6])
# 3 TEs found

ID_ins_sig_TE_CMT2 <- as.vector(ID_insertions[ID_insertions$TE %in% sig_TE_CMT2$geneID, 5])
# 15 TEs found

ID_ins_sig_TE_FBX5 <- as.vector(ID_insertions[ID_insertions$TE %in% sig_TE_FBX5$geneID, 5])
# 12 TEs found

```


Show TE activity for these subset:

```{r}

print("CMT2 TE deletions")
mean(rowSums(matrix_deletions_SA[ID_del_sig_TE_CMT2, allele_id$CMT2 == "0"]))
mean(rowSums(matrix_deletions_SA[ID_del_sig_TE_CMT2, allele_id$CMT2 == "1"]))

print("FBX5 TE deletions")
mean(rowSums(matrix_deletions_SA[ID_del_sig_TE_FBX5, allele_id$FBX5 == "0"]))
mean(rowSums(matrix_deletions_SA[ID_del_sig_TE_FBX5, allele_id$FBX5 == "1"]))

print("CMT2 TE insertions")
mean(rowSums(matrix_insertions_SA[ID_ins_sig_TE_CMT2, allele_id$CMT2 == "0"]))
mean(rowSums(matrix_insertions_SA[ID_ins_sig_TE_CMT2, allele_id$CMT2 == "1"]))

print("FBX5 TE insertions")
mean(rowSums(matrix_insertions_SA[ID_ins_sig_TE_FBX5, allele_id$FBX5 == "0"]))
mean(rowSums(matrix_insertions_SA[ID_ins_sig_TE_FBX5, allele_id$FBX5 == "1"]))
```
```{r}

print("CMT2 TE deletions")
mean(rowSums(matrix_deletions_SA[sample(ID_deletions$ID, 9), allele_id$CMT2 == "0"]))
mean(rowSums(matrix_deletions_SA[sample(ID_deletions$ID, 9), allele_id$CMT2 == "1"]))

print("FBX5 TE deletions")
mean(rowSums(matrix_deletions_SA[sample(ID_deletions$ID, 3), allele_id$FBX5 == "0"]))
mean(rowSums(matrix_deletions_SA[sample(ID_deletions$ID, 3), allele_id$FBX5 == "1"]))

print("CMT2 TE insertions")
mean(rowSums(matrix_insertions_SA[sample(ID_insertions$ID, 15), allele_id$CMT2 == "0"]))
mean(rowSums(matrix_insertions_SA[sample(ID_insertions$ID, 15), allele_id$CMT2 == "1"]))

print("FBX5 TE insertions")
mean(rowSums(matrix_insertions_SA[sample(ID_insertions$ID, 12), allele_id$FBX5 == "0"]))
mean(rowSums(matrix_insertions_SA[sample(ID_insertions$ID, 12), allele_id$FBX5 == "1"]))
...
Install the RELATE software from https://myersgroup.github.io/relate/ (see paper https://www.nature.com/articles/s41588-019-0484-x).
...
Install software Clues on GitHub https://github.com/standard-aaron/clues (see paper https://doi.org/10.1371/journal.pgen.1008384).
...
* **Emmanuel Tergemina** [EmmanuelTergemina](https://github.com/EmmanuelTergemina)
